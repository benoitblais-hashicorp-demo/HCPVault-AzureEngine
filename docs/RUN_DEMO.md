...
Provisions HCP Vault resources including Azure secrets engine, dynamic and static credential roles, AppRole authentication backend, and
least-privilege policies. Created when variable `enable_demo_resources` is set to `true`.
...
Core variables: `azure_client_id`, `azure_client_secret`, `azure_subscription_id`, `azure_tenant_id`. Azure AD Application Object IDs:
`azure_dynamic_spn_object_id` and `azure_static_spn_object_id` must be configured for the respective credential roles.
...
Before running the demo scripts, you must retrieve the AppRole credentials that applications will use to authenticate to Vault. The Role
ID (static identifier) is obtained from Terraform outputs, while the Secret ID (dynamic, short-lived credential) must be generated
on-demand using the Vault CLI. These two pieces together form the AppRole authentication credentials required by the demo scripts.
